...
void apagaArvore(noArvore **Tree);
...
void apagaArvore(noArvore **Tree){
    noArvore * aux;
    if(*Tree != NULL){                                                          
        apagaArvore(&((*Tree)->esquerda));
        apagaArvore(&((*Tree)->direita));
        aux = *Tree;
        *Tree = NULL;
        free(aux);
    }
...
//n sei
int contaNosArvore(noArvore **Tree, int *qtdNos){
    if(*Tree != NULL){
        contaNosArvore(&((*Tree)->esquerda), qtdNos);
        (*qtdNos)++;
        contaNosArvore(&((*Tree)->direita), qtdNos);
        return (*qtdNos);
    }
}
